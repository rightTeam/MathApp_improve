...
public class PlayActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final Animation alpha = AnimationUtils.loadAnimation(PlayActivity.this,
				R.anim.a1);
		...
		for (int i = 0; i < 3; i++) {
			...
			if (i != whichtrue) {
				try {
					float staticString = Float.parseFloat(string)
							+ (int) (Math.random() * 20 + 1);
					answer[i].setText(String.valueOf(decimal
							.format(staticString)));// 将答案加一个随机数编程错误答案
					...
					int data1_1 = Integer.parseInt(fenxi.nextToken())
							+ (int) (Math.random() * 5 + 1);// 将答案加一个随机数编程错误答案
					int data1_2 = Integer.parseInt(fenxi.nextToken())
							+ (int) (Math.random() * 5 + 1);
					String strfenshui = String.valueOf(data1_1) + "/"
							+ String.valueOf(data1_2);
		...
		answer[0].setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (count < 9) {
					...
					for (int i = 0; i < 3; i++) {
						...
						if (i != whichtrue) {
							try {
								...
								StringTokenizer fenxi = new StringTokenizer(
										string, "/");
								int data1_1 = Integer.parseInt(fenxi
										.nextToken())
										+ (int) (Math.random() * 5 + 1);// 将答案加一个随机数编程错误答案
								int data1_2 = Integer.parseInt(fenxi
										.nextToken())
										+ (int) (Math.random() * 5 + 1);
								String strfenshui = String.valueOf(data1_1)
										+ "/" + String.valueOf(data1_2);
					...
					if (total >= 6) {
						mediaPlayer.stop();
						questionTextView.setText("恭喜!恭喜! " + "答对的了" + total
								+ "道题哦");
						soundPool.play(result, 1, 1, 0, 0, 1);
						passView.setVisibility(View.VISIBLE);
					} else if (total < 6) {
						mediaPlayer.stop();
						questionTextView.setText("很遗憾!哟~" + "只答对的了" + total
								+ "道题哦");
						soundPool.play(result, 1, 1, 0, 0, 1);
	...
	public static void timu() {// 题目
		...
		for (int i = 0; i < 10; i++) {// 10道题
			...
			} else if (ccs == 1) // 阶层式子
			{
				...
				data1 = Integer.toString(num3[0]) + "/"
						+ Integer.toString(num3[1]);
				data2 = Integer.toString(num3[2]) + "/"
						+ Integer.toString(num3[3]);
				data3 = Integer.toString(num3[4]) + "/"
						+ Integer.toString(num3[5]);
				...
				if (khcount == 1) {// 一对括号
					...
					if (randposition == 1) {
						khdata = "(" + String.valueOf(num4[0])
								+ String.valueOf(op[no[0]])
								+ String.valueOf(num4[1]) + ")"
								+ String.valueOf(op[no[1]])
								+ String.valueOf(num4[2])
								+ String.valueOf(op[no[3]])
								+ String.valueOf(num4[3]);
						s[i] = khdata + "=?";
